...
class getFitness(object):
    """getFitness class definition. Inverse of time taken to move from start node to
       goal node (test scenario) is the measure of fitness passed to the genetic 
       algorithm.                           
    """
    ...
    def __init__(self, config):
        ...
        self.goal_node = config["goal_node"]
        self.robot_name = config["robot_name"]
        self.max_wait_time = rospy.Duration(config["max_wait_time"])
    ...
    def run_scenario(self, parameters=[]):
        """Run the test scenario with parameters given by the GA and get fitness 
        ...
        try:
            ...
            # TO DO: DYNAMIC RECONFIGURE HERE
            ...
            if result.success:
                ...
                print "Failed to complete scenario in maximum alloted time of \
                {} seconds.".format(t)
...
if __name__ == "__main__":
    ...
    config_file = find_resource("rasberry_optimise", "scenario.yaml")[0]
    ...
    gf = getFitness(config)    
    
    # Run the genetic algorithm here with `gf.run_scenario` as the fitness function.
    gf.run_scenario()
    #gf.run_scenario()
#####################################################################################