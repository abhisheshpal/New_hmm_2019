...
tmux new-window -t $SESSION:2 -n 'localisation'
tmux new-window -t $SESSION:3 -n 'move_base'
tmux new-window -t $SESSION:4 -n 'topo_nav'
tmux new-window -t $SESSION:5 -n 'rviz'
tmux new-window -t $SESSION:6 -n 'marvel'
tmux new-window -t $SESSION:7 -n 'coordination'
...
tmux send-keys "DISPLAY=:$DISPLAY roslaunch rasberry_bringup robot_bringup.launch robot_model:=$(rospack find rasberry_bringup)/config/norway_robot.yaml model_extras:=$(rospack find rasberry_bringup)/urdf/norway_robot_sensors.xacro simple_sim:=false"
