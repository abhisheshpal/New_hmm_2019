...
__global__ void Init_Cluster_Centers_device(const Vector4f* inimg, const Vector4u* src_inimg, spixel_info* out_spixel, Vector2i map_size, Vector2i img_size, int spixel_size);
...
__global__ void Update_Cluster_Center_device(const Vector4f* inimg, const Vector4u* src_inimg, const int* in_idx_img, spixel_info* accum_map, Vector2i map_size, Vector2i img_size, int spixel_size, int no_blocks_per_line);
...
__global__ void Draw_Bw_Contour_device(const int* idx_img, int* bw_contour_img_ptr, Vector2i img_size, int apple_id, Vector4u* gpucluster, bool flag_for_mixing);
...
seg_engine_GPU::seg_engine_GPU(const settings& in_settings) : seg_engine(in_settings)
{
	...
	gpu_cluster = new UChar4Image(Vector2i(spixel_per_col*spixel_per_row, 1), true, true);
...
void gSLICr::engines::seg_engine_GPU::Init_Cluster_Centers()
{
	...
	Vector4u* src_img_ptr = source_img->GetData(MEMORYDEVICE_CUDA);
	...
	Init_Cluster_Centers_device << <gridSize, blockSize >> >(img_ptr, src_img_ptr, spixel_list, map_size, img_size, spixel_size);
}
...
void gSLICr::engines::seg_engine_GPU::Update_Cluster_Center()
{
	...
	Update_Cluster_Center_device<<<gridSize,blockSize>>>(img_ptr, src_img_ptr, idx_ptr, accum_map_ptr, map_size, img_size, spixel_size, no_blocks_per_line);
...
void gSLICr::engines::seg_engine_GPU::Draw_Bw_Contour(IntImage* bw_contour, int apple_id, bool flag_for_mixing)
{
	int* idx_img_ptr = idx_img->GetData(MEMORYDEVICE_CUDA);
	int* bw_contour_img_ptr = bw_contour->GetData(MEMORYDEVICE_CUDA);
	Vector4u* gpucluster = gpu_cluster->GetData(MEMORYDEVICE_CUDA);
	...
	Draw_Bw_Contour_device<<<gridSize,blockSize>>>(idx_img_ptr, bw_contour_img_ptr, img_size, apple_id, gpucluster, flag_for_mixing);
	bw_contour->UpdateHostFromDevice();
}
...
__global__ void Draw_Bw_Contour_device(const int* idx_img, int* bw_contour_img, Vector2i img_size, int apple_id, Vector4u* gpucluster, bool flag_for_mixing)
{
	int x = threadIdx.x + blockIdx.x * blockDim.x, y = threadIdx.y + blockIdx.y * blockDim.y;
	if (x == 0 || y == 0 || x > img_size.x - 2 || y > img_size.y - 2) return;

	draw_bw_contour_shared(idx_img, bw_contour_img, img_size, x, y, apple_id, gpucluster, flag_for_mixing);
}
...
__global__ void Init_Cluster_Centers_device(const Vector4f* inimg, const Vector4u* src_inimg, spixel_info* out_spixel, Vector2i map_size, Vector2i img_size, int spixel_size)
{
	...
	init_cluster_centers_shared(inimg, src_inimg, out_spixel, map_size, img_size, spixel_size, x, y);
}
...
__global__ void Update_Cluster_Center_device(const Vector4f* inimg, const Vector4u* src_inimg, const int* in_idx_img, spixel_info* accum_map, Vector2i map_size, Vector2i img_size, int spixel_size, int no_blocks_per_line)
{
	...
	__shared__ Vector2f red_color_shared[BLOCK_DIM*BLOCK_DIM];
	...
	red_color_shared[local_id] = Vector2f(0, 0);
	...
	if (x_offset < spixel_size * 3 && y_offset < spixel_size * 3)
	{
		...
		if (x_img >= 0 && x_img < img_size.x && y_img >= 0 && y_img < img_size.y)
		{
			...
			if (in_idx_img[img_idx] == spixel_id)
			{
				...
				red_color_shared[local_id].x = 2*src_inimg[img_idx].x -src_inimg[img_idx].y -src_inimg[img_idx].z ;
	...
	if (should_add)
	{
		if (local_id < 128)
		{
			...
			red_color_shared[local_id] += red_color_shared[local_id + 128];
		...
		if (local_id < 64)
		{
			...
			red_color_shared[local_id] += red_color_shared[local_id + 64];
		...
		if (local_id < 32)
		{
			...
			red_color_shared[local_id] += red_color_shared[local_id + 32];
			red_color_shared[local_id] += red_color_shared[local_id + 16];
			red_color_shared[local_id] += red_color_shared[local_id + 8];
			red_color_shared[local_id] += red_color_shared[local_id + 4];
			red_color_shared[local_id] += red_color_shared[local_id + 2];
			red_color_shared[local_id] += red_color_shared[local_id + 1];
	...
	if (local_id == 0)
	{
		...
		accum_map[accum_map_idx].red_intensity.x = red_color_shared[0].x;
