...
_CPU_AND_GPU_CODE_ inline void init_cluster_centers_shared(const gSLICr::Vector4f* inimg, const gSLICr::Vector4u* src_inimg, gSLICr::objects::spixel_info* out_spixel, gSLICr::Vector2i map_size, gSLICr::Vector2i img_size, int spixel_size, int x, int y)
{
	...
	out_spixel[cluster_idx].red_intensity.x = 2*src_inimg[img_y*img_size.x + img_x].x - src_inimg[img_y*img_size.x + img_x].y - src_inimg[img_y*img_size.x + img_x].z;
...
_CPU_AND_GPU_CODE_ inline void draw_bw_contour_shared(const int* idx_img, int* bw_contour, gSLICr::Vector2i img_size, int x, int y, int apple_id, gSLICr::Vector4u* gpucluster, bool flag_for_mixing)
{
	int idx = y * img_size.x + x;

	if(flag_for_mixing == 0) //that is we don't want to mix. Reinit everypixel to zero.
	bw_contour[idx] = 0;

	if (gpucluster[idx_img[idx]].b == apple_id)
		bw_contour[idx] = 255;
}
...
_CPU_AND_GPU_CODE_ inline void finalize_reduction_result_shared(const gSLICr::objects::spixel_info* accum_map, gSLICr::objects::spixel_info* spixel_list, gSLICr::Vector2i map_size, int no_blocks_per_spixel, int x, int y)
{
	...
	spixel_list[spixel_idx].red_intensity = gSLICr::Vector2f(0, 0);
	...
	for (int i = 0; i < no_blocks_per_spixel; i++)
	{
		...
		spixel_list[spixel_idx].red_intensity += accum_map[accum_list_idx].red_intensity;
	...
	if (spixel_list[spixel_idx].no_pixels != 0)
	{
		...
		spixel_list[spixel_idx].red_intensity.x /= (float)spixel_list[spixel_idx].no_pixels;
	}
}
