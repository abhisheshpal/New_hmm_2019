...
void seg_engine::Perform_Segmentation(UChar4Image* in_img)
{
	...
	/*
		Here we are changing the color space from bgr to cvt. All the
		followed work is going to be done using the cvt_img.
	*/
...
void seg_engine::Push_Cluster_Data(UChar4Image* clusters)
{
	gpu_cluster->SetFrom(clusters, ORUtils::MemoryBlock<Vector4u>::CPU_TO_CUDA);
}
