...
void gSLICr::engines::core_engine::Push_Cluster_Data(UChar4Image* clusters)
{
	slic_seg_engine->Push_Cluster_Data(clusters);
}
...
void gSLICr::engines::core_engine::Draw_Bw_Contour(IntImage* bw_contour, int apple_id, bool flag_for_mixing)
{
	slic_seg_engine->Draw_Bw_Contour(bw_contour, apple_id, flag_for_mixing);
}
