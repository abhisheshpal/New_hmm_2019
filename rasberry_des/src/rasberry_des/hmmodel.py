...
#        (t_seq1, s_seq1, e_seq1) = generate_random(len(obs), 1)
        ...
        post_distribution = numpy.exp( log_prob_table[-1] )
        ...
        if verbose:
            ...
            print( "Probability of being generated by the found state sequence:", numpy.exp( log_prob ) )
#            print( "state probs: \n", post_distribution)

        uniform = numpy.array([1.0 / self.n_states] * self.n_states)   # get the 1 X N_nodes matrix of uniform distribution
        ...
        D_KL = numpy.sum(numpy.multiply(post_distribution, numpy.log(numpy.divide(post_distribution, uniform))))
    ...
    def check_prediction_probs(self, obs, forecast_max, forecast_steps, verbose=False):
        ...
        for i in numpy.linspace(1, forecast_max, forecast_steps):  # Return evenly spaced numbers over a specified interval (start,stop, Num = int).
