...
#                    self.yield_at_node[node_id] = (node_yield_in_row[row_id] * last_node_dist) / row_node_dist
    ...
    def get_closest_nodes_while_picking(self, row_id, node, pick_dir, distance):
        ...
        if pick_dir == "reverse":
            ...
            if row_id in self.half_rows:
                ...
                route_nodes, _, route_dists = self.get_path_details(node, self.row_info[row_id][2])
                route_dir = ["forward" for i in range(len(route_nodes))]
            else:
                # full-row, forward and reverse picking
                _route_nodes, _, _route_dists = self.get_path_details(node, self.row_info[row_id][2])
                route_nodes.extend(_route_nodes)
                route_dists.extend(_route_dists)
                route_dir = ["forward" for i in range(len(_route_nodes))]
                _route_nodes, _, _route_dists = self.get_path_details(self.row_info[row_id][2], self.row_info[row_id][1])
