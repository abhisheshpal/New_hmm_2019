...
def check_fork_map_config(ns):
    ...
    missing_params = []
    if not rospy.has_param(ns + "n_farm_rows"):
        missing_params.append("n_farm_rows")
    if not rospy.has_param(ns + "half_rows"):
        missing_params.append("half_rows")
    if not rospy.has_param(ns + "head_row_node_dist"):
        missing_params.append("head_row_node_dist")
    if not rospy.has_param(ns + "head_node_y"):
        missing_params.append("head_node_y")
    if not rospy.has_param(ns + "row_node_dist"):
        missing_params.append("row_node_dist")
    if not rospy.has_param(ns + "row_length"):
        missing_params.append("row_length")
    if not rospy.has_param(ns + "row_spacing"):
        missing_params.append("row_spacing")
    return missing_params

def check_des_config(ns):
    ...
    missing_params = check_fork_map_config(ns)
    if not rospy.has_param(ns + "n_pickers"):
        missing_params.append("n_pickers")
    if not rospy.has_param(ns + "picking_rate"):
        missing_params.append("picking_rate")
    if not rospy.has_param(ns + "transportation_rate"):
        missing_params.append("transportation_rate")
    if not rospy.has_param(ns + "max_n_trays"):
        missing_params.append("max_n_trays")
    if not rospy.has_param(ns + "loading_time"):
        missing_params.append("loading_time")
    if not rospy.has_param(ns + "tray_capacity"):
        missing_params.append("tray_capacity")
    if not rospy.has_param(ns + "yield_per_node"):
        missing_params.append("yield_per_node")
    return missing_params

def get_fork_map_config_parameters(ns):
    # assume check_des_config_parameters.py is run and all parameters are present
    n_farm_rows = rospy.get_param(ns + "n_farm_rows")
    ...
    half_rows = rospy.get_param(ns + "half_rows")
    ...
    _head_row_node_dist = rospy.get_param(ns + "head_row_node_dist")
    if (_head_row_node_dist.__class__ == list):
        ...
        if (len(_head_row_node_dist) == n_topo_nav_rows) or (len(_head_row_node_dist) == 1):
            ...
            rospy.ROSException("%shead_row_node_dist must either be an array of size %d or 1" %(ns, n_topo_nav_rows))
    else:
        rospy.ROSException("%shead_row_node_dist must either be an array of size %d or 1" %(ns, n_topo_nav_rows))

    _head_node_y = rospy.get_param(ns + "head_node_y")
    if (_head_node_y.__class__ == list):
        # if there are n_rows values, take
        if (len(_head_node_y) == n_topo_nav_rows) or (len(_head_node_y) == 1):
            head_node_y = _head_node_y
        else:
            rospy.ROSException("%shead_node_y must either be an array of size %d or 1" %(ns, n_topo_nav_rows))
    else:
        rospy.ROSException("%shead_node_y must either be an array of size %d or 1" %(ns, n_topo_nav_rows))

    _row_node_dist = rospy.get_param(ns + "row_node_dist")
    if (_row_node_dist.__class__ == list):
        ...
        if (len(_row_node_dist) == n_topo_nav_rows) or (len(_row_node_dist) == 1):
            ...
            rospy.ROSException("%(ns, ns)row_node_dist must either be an array of size %d or 1" %(ns, n_topo_nav_rows))
    else:
        rospy.ROSException("%srow_node_dist must either be an array of size %d or 1" %(ns, n_topo_nav_rows))
    ...
    _row_length = rospy.get_param(ns + "row_length")
    if (_row_length.__class__ == list):
        # if there are n_rows values, take
        if (len(_row_length) == n_topo_nav_rows) or (len(_row_length) == 1):
            row_length = _row_length
        else:
            rospy.ROSException("%srow_length must either be an array of size %d or 1" %(ns, n_topo_nav_rows))
    else:
        rospy.ROSException("%srow_length must either be an array of size %d or 1" %(ns, n_topo_nav_rows))
    ...
    _row_spacing = rospy.get_param(ns + "row_spacing")
    if (_row_length.__class__ == list):
        # if there are n_rows values, take
        if (len(_row_spacing) == n_topo_nav_rows) or (len(_row_spacing) == 1):
            row_spacing = _row_spacing
        else:
            rospy.ROSException("%srow_spacing must either be an array of size %d or 1" %(ns, n_topo_nav_rows))
    else:
        rospy.ROSException("%srow_spacing must either be an array of size %d or 1" %(ns, n_topo_nav_rows))
...
def get_des_config_parameters(ns, map_from_db=False):
    ...
    if not map_from_db:
        ...
        config_params = list(get_fork_map_config_parameters(ns))
    ...
    des_env = rospy.get_param(ns + "des_env")

    n_pickers = rospy.get_param(ns + "n_pickers")

    _picking_rate = rospy.get_param(ns + "picking_rate")
    if (_picking_rate.__class__ == list):
        if (len(_picking_rate) == n_pickers) or (len(_picking_rate) == 1):
            picking_rate = _picking_rate
        else:
            rospy.ROSException("%spicking_rate must either be an array of size %d or 1" %(ns, n_pickers))
    else:
        rospy.ROSException("%spicking_rate must either be an array of size %d or 1" %(ns, n_pickers))

    _transportation_rate = rospy.get_param(ns + "transportation_rate")
    if (_transportation_rate.__class__ == list):
        if (len(_transportation_rate) == n_pickers) or (len(_transportation_rate) == 1):
            ...
            rospy.ROSException("%stransportation_rate must either be an array of size %d or 1" %(ns, n_pickers))
    else:
        rospy.ROSException("%stransportation_rate must either be an array of size %d or 1" %(ns, n_pickers))

    _max_n_trays = rospy.get_param(ns + "max_n_trays")
    if (_max_n_trays.__class__ == list):
        if (len(_max_n_trays) == n_pickers) or (len(_max_n_trays) == 1):
            max_n_trays = _max_n_trays
        else:
            rospy.ROSException("%smax_n_trays must either be an array of size %d or 1" %(ns, n_pickers))
    else:
        rospy.ROSException("%smax_n_trays must either be an array of size %d or 1" %(ns, n_pickers))

    _loading_time = rospy.get_param(ns + "loading_time")
    if (_loading_time.__class__ == list):
        if (len(_loading_time) == n_pickers) or (len(_loading_time) == 1):
            loading_time = _loading_time
        else:
            rospy.ROSException("%sloading_time must either be an array of size %d or 1" %(ns, n_pickers))
    else:
        rospy.ROSException("%sloading_time must either be an array of size %d or 1" %(ns, n_pickers))

    tray_capacity = rospy.get_param(ns + "tray_capacity")

    _yield_per_node = rospy.get_param(ns + "yield_per_node")
    if (_yield_per_node.__class__ == list):
        if (len(_yield_per_node) == n_topo_nav_rows) or (len(_yield_per_node) == 1):
            ...
            rospy.ROSException("%syield_per_node must either be an array of size %d or 1" %(ns, n_topo_nav_rows))
    else:
        rospy.ROSException("%syield_per_node must either be an array of size %d or 1" %(ns, n_topo_nav_rows))
