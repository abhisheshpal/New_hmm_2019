...
#    state_map[16, :] = np.multiply(state_map[16,:], 0.0050954)
#    state_map[17, :] = np.multiply(state_map[17,:], 0.0050954)
#    state_map[18, :] = np.multiply(state_map[18,:], 0.0050954)
#    state_map[19, :] = np.multiply(state_map[19,:], 0.0050954)
#    state_map[20, :] = np.multiply(state_map[20,:], 0.0050954)
#    state_map[21, :] = np.multiply(state_map[21,:], 0.0050954)
...
#    state_map[9, :] = np.multiply(state_map[9,:], 0.00175)
    ...
    B_pre = np.ones(n_states) * .006 + np.eye(n_states) * .7 + np.eye(n_states, k=1) * .1 + np.eye(n_states, k=-1) * .1
...
#
#    # sample a random sequence within desired time peroiod from the above created model(for testing and generation)
#    t_seq, s_seq, e_seq = mode_model.generate_random(sample_len=200, sample_step=1)
#
#    # predict for a specific time from an initial observation
#    (state, KL, posteriors) = mode_model.predict(
#                                                 # start with some observations assumed to have made up to a point
