...
class TopoMapPlotter(object):
    ...
    def plot_map(self, ):
        ...
        for node_name in self.node_names:
            x = self.nodes[node_name].pose.position.x
            y = self.nodes[node_name].pose.position.y
            ax.text(x-x_delta/4.0, y + y_delta/4.0, node_name, fontsize=10)
...
if __name__ == "__main__":
    rospy.init_node("create_topo_map_plot")

    if len(sys.argv) < 2:
        rospy.loginfo("Usage is create_topo_map_plot path_to_plot_file")
    else:
        fig_file = os.path.abspath(sys.argv[1])


    try:
        map_plotter = TopoMapPlotter(fig_file)
    except:
